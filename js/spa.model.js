...
spa.model = (function(){ 
	...
	// The people object provides methods and enents to manage
	// a collection of person objects. Its public methods include
	//  * get_user() - return the current user person obect.
	//    If the current user is not singed-in, an anonymous person object
	//    is returned
	//  * get_db() - return the TaffyDB databases of all the person object
	//             - including the current user - presorted.
	//  * get_by_cid(<client_ic>) - return a person object with provided unique id.
	...
	//    Successful complition of login publishes a 'spa-login' global custom event.
	...
	//    The former user object is provided as user.//former user前用户 
	//
	...
	//  * cid - string client id. This is always degined, and 
	//    is only different from the it attribute 
	//    if the client data is no synece withd the backend.//如果客户端数据没有和后台同步
	//  * id - the unique id. This may be undefined if the 
	//    object is not synced with the backend.
	//  * name - the string name of the user.
	//  * css_map - a map of attributes used for avatar presentation // 用于头像描述的属性MAP
	...
	// The chat object provies methods and events to manage chat messaging.
	// Its public methods include:
	// * join() - joins the chat room. This routine sets up the chat protocol
	//   with the backend includeing publishers for 'spa-listchange' and
	...
	// * set_charee(<person_id>) - set the chatee to the person identified by
	//   person_id. if the person_id dose not exit in the person list, the 
	//   chatee is set to null. If the person requested is already the chatee,
	//   it returns false. It publishes a 'spa-setchatee' global custom event.
	...
	//   people list and avatar information(the css_map in the person objects). The
	//   update_avtr_map mush have the form{person_id:person_id, css_map: css_map}.
	//
	// jQuery global custom events published by the object include:
	// * spa-setchatee - This is published when a new chatee is set.
	//   a map of the form:
	//   { old_chatee: <old_chatee_person_object>
	//	   new_chatee: <new_chatee_person_object>
	//   }
	//   is provided as data.
	// * spa-listchange - This is published when the list of online people changes
	//   in length(i.e. when a person joins or leaves a chat) or when their contents
	//   change(i.e. when a person's avatar details change).
	//   A subscriber to this event should get the people_db from the people for
	//   the updated data.
